    ...
    homeModules = self.lib.mkEnableModule {
      ...
      config = {
        ...
        programs.niri = {
          ...
          package = pkgs.niri;
          settings = let
            scripts = self'.packages.niri-scripts;
            listToIndexedAttrs = {padKeys ? false}: list: let
              len = list |> builtins.length |> builtins.toString |> builtins.stringLength;
              padFn =
                if padKeys
                then
                  self.lib.string.pad {
            ...
            in
              list
              |> lib.imap0 (i: value: {
                name = i |> builtins.toString |> padFn;
                inherit value;
              })
          ...
          in {
            ...
            binds = explicitRepeat {
              ...
              "Mod+Shift+Return" = {
                ...
                action.spawn-sh = (self.lib.option.ensureNotNull "Enable a default launcher" configNamespace.settings.defaultApps.launcher).exec;
              };
